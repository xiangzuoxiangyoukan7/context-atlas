...
当前源码清单版本与知识库格式版本均为 `0.20.2`；同一发布的 Git tag、插件清单和知识库 `format_version` 必须一致。
...
Agent 调研并展示 Proposal，用户确认精确的 `proposal_revision`，确定性执行器才会写入并验证。知识库升级由 Agent 判断需要理解上下文的等价转换，Python 执行器负责运行资产、路径边界、摘要、隔离预演、正式应用和验证；预演未通过时保持正式知识库零写入。普通自然语言、外部任务完成状态、测试通过或检查器通过，都不能代替用户确认。
...
正式知识使用 Markdown 和 YAML Front Matter。`knowledge-base.yaml` 是机器入口，`schemas/catalog.json` 与各类型 Schema 是格式权威，`.project-kb/scripts/check_knowledge_base.py` 负责确定性结构验证。当前初始化格式为 `format_version: 0.20.2`，与插件发布版本一致。`knowledge_revision` 由执行器在正式事务成功后递增；新知识库不再生成全局 `project_version`。决策依据保存在所属需求、功能、技术或治理文档中，不建立独立 ADR；需求以 Markdown 正文保存业务内容，Front Matter 只保留机器身份、状态、分类和更新时间。
