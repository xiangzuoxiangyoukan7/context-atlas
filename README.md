...
当前源码清单版本为 `0.11.1`。源码清单版本不等于各 Marketplace 已发布版本，安装或升级后必须以宿主实际显示的版本为准。
...
当前初始化知识库使用 `format_version: 7`。根目录的 `knowledge-base.yaml` 至少记录协议版本、Schema 版本、项目稳定编号与名称、知识库名称、工作区模式、项目版本、磁盘格式版本、知识库修订号、初始化时间以及各类权威入口。
...
Context Atlas 不阻止用户开始开发，但建议先用 `navigate` 查询影响，再用 `review` 检查范围、边界和验收标准。关键信息缺失时应先确认，避免 Agent 用推测补齐需求。
...
当前源码清单版本为 `0.11.1`，Codex、Claude Code 和 Qoder 共享产品名 `context-atlas`。源码清单版本不等于各 Marketplace 已发布版本；安装或升级后必须以宿主实际显示的版本为准。平台发布包由当前源码仓库构建，不维护平台专属源码分叉。
...
git -C D:\loong-workspace-python\context-atlas-claude-plugin commit -m "release: context-atlas 0.11.1"
...
git -C D:\loong-workspace-python\context-atlas-qoder-plugin commit -m "release: context-atlas 0.11.1"
