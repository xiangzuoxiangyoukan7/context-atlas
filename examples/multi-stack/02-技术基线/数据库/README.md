...
- 数据源：实例、集群连接、只读副本或数据仓库，不保存凭据值。
- 数据库单元：Database、Oracle PDB 或逻辑数据库。
- 数据命名空间：Schema、用户模式或 Namespace；产品没有独立层级时不创建空壳。
- 数据表：项目实际读写或依赖的业务表，一表一文件。
...
知识项模板集中在 .project-kb/templates/knowledge/，通过受控创建操作使用。
